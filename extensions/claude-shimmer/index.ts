...
function estimateBlockTokenUnits(
	block: NonNullable<AssistantTokenMessage["content"]>[number],
): number {
...
function sampleMacaron(pos: number): [number, number, number] {
	...
	const from = stops[i] ?? SAKURA;
	const to = stops[i + 1] ?? SKY;
	return blend(from, to, scaled - i);
}
...
export default function (pi: ExtensionAPI) {
	...
	function buildStatusParts(): string[] {
		...
		if (currentReportedTokens !== null || currentEstimatedTokens > 0 || tokens > 0) {
	...
	function appendEstimatedBlock(index: number, text: string) {
		setEstimatedBlock(
			index,
			(currentBlockTokenUnits.get(index) ?? 0) + estimateTextTokenUnits(text),
		);
	}
	...
	pi.on("message_update", async (event, ctx) => {
		...
		switch (evt.type) {
			...
			case "thinking_end":
				...
				break;
			case "toolcall_start": {
				const block = tokenMessage.content?.[evt.contentIndex];
				setEstimatedBlock(evt.contentIndex, block ? estimateBlockTokenUnits(block) : 0);
				break;
			}
			case "toolcall_end": {
	...
	pi.on("message_end", async (event, ctx) => {
		...
		_displayedTokens = Math.max(0, completedOutputTokens);
		_tokensMoving = false;
		updateDisplay();
	});
	...
	pi.on("agent_end", async () => {
		...
		if (ctx_?.ui) {
			...
			ctx_.ui.notify(`${glyph}${body}`, "info");
		}
	});
