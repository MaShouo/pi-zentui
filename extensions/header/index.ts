...
function renderHeader(width: number, availableRows = 0): string[] {
	...
	const railPad = " ".repeat(
		Math.max(0, Math.min(width - railWidth, Math.floor((width - railWidth) / 2) + 1)),
	);
	const visibleTelemetry = [...telemetry].slice(0, width).join("");
	const telemetryWidth = [...visibleTelemetry].length;
	const telemetryPad = " ".repeat(
		Math.max(0, Math.min(width - telemetryWidth, Math.floor((width - telemetryWidth) / 2) + 1)),
	);
