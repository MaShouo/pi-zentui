...
import { type RGB, renderSakuraGradient } from "./gradient";
...
export function parseDiffLine(
	plain: string,
): { kind: "add" | "del" | "ctx"; lineNum: string; content: string } | { kind: "ellipsis" } | null {
...
function looksLikePathLine(plain: string): boolean {
	...
	if (
		/\.(?:ts|tsx|js|jsx|mjs|cjs|json|md|py|rs|go|css|html|sh|toml|yaml|yml)$/i.test(s) &&
		!/\s/.test(s)
	) {
		return true;
	}
...
function summarizeHugePayload(
	...
	const cmd =
		plains.slice(0, 6).find((p) => /\b(cat|tee|write|edit|node|python|bash)\b/.test(p)) ?? first;
	const pathMatch = plains
		.join("\n")
	...
	head.push(
		themeOr(
			...
			"dim",
			DIM_RGB,
			`${lines} lines · ~${Math.round(bytes / 1024)}KB · expand to show`,
		),
	);
	return head;
}
...
export function compactToolBody(
	...
	if (isHeredocOrHugeWrite(plains)) {
		...
		const titleLine =
			firstPlain && !firstPlain.includes("{")
				? (styleToolTitle(firstPlain, theme) ?? themeOr(theme, "toolTitle", SKY, firstPlain))
				: undefined;
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (plain.length > TOOL_COLLAPSED_MAX_LINE_CHARS) {
			...
			out.push(
				line.includes("\x1b[")
					? themeOr(theme, "toolOutput", MUTED, cut)
					: styleGenericBodyLine(cut, cut, theme),
			);
		} else {
			out.push(line);
		}
...
export function renderStatsChip(stats: DiffStats, theme?: ThemeLike): string | undefined {
	...
	const del =
		stats.removed > 0 ? themeOr(theme, "toolDiffRemoved", CORAL, `−${stats.removed}`) : "";
